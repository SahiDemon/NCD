...
// Ordered list of selectors to try when injecting the NDC UI.
// NexusMods occasionally restructures their DOM; add new selectors here as needed.
const NDC_CONTAINER_SELECTORS = [
    // Current layout (2025-2026): right-hand sidebar on collection pages
    "aside.collection-page-sidebar",
    "[class*='collection-page-sidebar']",
    // Older "next-container" layout
    "#mainContent > div > div.relative > div.next-container",
    // Generic right sidebar fallbacks
    "#mainContent aside",
    ".collection-detail aside",
    // Broadest fallback: first aside anywhere inside main content
    "#mainContent aside:first-of-type",
];

function findNdcContainer() {
    for (const selector of NDC_CONTAINER_SELECTORS) {
...
function injectNdcElement(ndcInstance, timeoutMs = 10000) {
    // Try immediately first
    const container = findNdcContainer();
    if (container) {
        container.prepend(ndcInstance.element);
        return;
    }

    // Container not yet in DOM (SPA navigation) — watch for it
    const deadline = Date.now() + timeoutMs;
    const retryObserver = new MutationObserver(() => {
        const found = findNdcContainer();
        if (found) {
            retryObserver.disconnect();
            found.prepend(ndcInstance.element);
        } else if (Date.now() > deadline) {
            retryObserver.disconnect();
            console.warn("[NDC] Could not find a container to inject UI into after", timeoutMs, "ms.");
        }
    });
